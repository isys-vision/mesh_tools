  ...
  */
  ...
  if (buffer->hasVertexColors()) {
    ...
    for (size_t i = 0; i < n_vertices; i++) {
      mesh_vertex_colors.vertex_colors[i].r =
          buffer_vertex_colors[i * 3 + 0] / 255.0;
      mesh_vertex_colors.vertex_colors[i].g =
          buffer_vertex_colors[i * 3 + 1] / 255.0;
      mesh_vertex_colors.vertex_colors[i].b =
          buffer_vertex_colors[i * 3 + 2] / 255.0;
  ...
  if (texture_cache) {
    ...
    for (unsigned int i = 0; i < n_textures; i++) {
      ...
      sensor_msgs::fillImage(image, "rgb8", buffer_textures[i].m_height,
...
bool fromMeshGeometryToMeshBuffer(
  ...
  if (!buffer_ptr)
    buffer_ptr = lvr2::MeshBufferPtr(new lvr2::MeshBuffer);
...
bool fromMeshGeometryToMeshBuffer(const mesh_msgs::MeshGeometry &mesh_geometry,
                                  lvr2::MeshBufferPtr &buffer_ptr) {
  if (!buffer_ptr)
    buffer_ptr = lvr2::MeshBufferPtr(new lvr2::MeshBuffer);
    ...
    for (int i = 0; i < old_numIndices; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            ...
            else
            {
                ...
                vertexMap.insert(pair<lvr::Vertex<float>, unsigned int>(vertex,
pos));
            }
...
static inline bool hasCloudChannel(const sensor_msgs::PointCloud2 &cloud,
...
bool fromPointCloud2ToPointBuffer(const sensor_msgs::PointCloud2 &cloud,
  ...
  for (int i = 0; iter_x_filter != iter_x_filter.end();
    ...
    if (!std::isnan(*iter_x_filter) && !std::isnan(*iter_y_filter) &&
  ...
  int index, i;
  for (i = 0, index = 0; iter_x != iter_x.end();
  ...
  bool normalsAvailable = hasCloudChannel(cloud, "normal_x") &&
  ...
  if (normalsAvailable) {
    ...
    int index, i;
    for (i = 0, index = 0; iter_n_x != iter_n_x.end();
         ++iter_n_x, ++iter_n_y, ++iter_n_z, index++) {
      // skip NaN point values
      if (!tmp_filter.empty() && index == tmp_filter.front()) {
        tmp_filter.pop_front();
        continue;
  ...
  if (hasCloudChannel(cloud, "rgb")) {
    ...
    tmp_filter = filter_nan;
    int index, i;
    for (i = 0, index = 0; iter_rgb != iter_rgb.end(); ++iter_rgb, index++) {
      // skip NaN point values
      if (!tmp_filter.empty() && index == tmp_filter.front()) {
        tmp_filter.pop_front();
        continue;
  ...
  if (hasCloudChannel(cloud, "intensities")) {
    ...
    tmp_filter = filter_nan;
    int index, i;
    for (i = 0, index = 0; iter_int != iter_int.end(); ++iter_int, index++) {
      // skip NaN point values
      if (!tmp_filter.empty() && index == tmp_filter.front()) {
        tmp_filter.pop_front();
        continue;
...
bool fromMeshGeometryMessageToMeshBuffer(
  ...
  if (mesh_geometry.vertex_normals.size() == mesh_geometry.vertices.size()) {
    ...
    ROS_ERROR_STREAM("Number of normals ("
                     << mesh_geometry.vertex_normals.size()
                     << ") must be equal to number of vertices ("
                     << mesh_geometry.vertices.size() << "), ignore normals!");
  }
  return true;
}

void PointBufferToPointCloud2(const lvr2::PointBufferPtr &buffer,
                              std::string frame,
  ...
  for (auto channelPair : floatChannels) {
    ...
    if (channelPair.first == "points") {
      ...
      int p_offset = 0;
      p_offset = addPointField(*cloud, "x", 1, sensor_msgs::PointField::FLOAT32,
                               p_offset);
      p_offset = addPointField(*cloud, "y", 1, sensor_msgs::PointField::FLOAT32,
                               p_offset);
      p_offset = addPointField(*cloud, "z", 1, sensor_msgs::PointField::FLOAT32,
                               p_offset);
      ...
      for (auto channelPair : uCharChannels) {
        if (channelPair.first == "colors") {
          ...
          offset = addPointField(*cloud, "rgb", channelPair.second.width(),
      ...
      offset = addPointField(*cloud, channelPair.first,
                             channelPair.second.width(), type + 1, offset);
      // int padding = (sizeof(float) * 4) - ((channelPair.second.width() *
      // sizeof(float)) % (4 * sizeof(float))); offset += padding;
      cloud->point_step = offset;
    }
  }
...
#pragma omp parallel for
          for (size_t i = 0; i < size; ++i) {
            unsigned char *ptr =
                &(cloud->data[cloud->point_step * i]) + field.offset;
            *(ptr + 2) = channel[i][0];
            *(ptr + 1) = channel[i][1];
            *(ptr + 0) = channel[i][2];
...
void PointCloud2ToPointBuffer(const sensor_msgs::PointCloud2Ptr &cloud,
    ...
    if (field.datatype == sensor_msgs::PointField::FLOAT32) {
      if (field.name == "x" || field.name == "y" || field.name == "z") {
        if (!buffer->hasFloatChannel("points")) {
          buffer->addEmptyChannel<float>("points", cloud->width * cloud->height,
                                         3);
        }
      } else {
        buffer->addEmptyChannel<float>(field.name, cloud->width * cloud->height,
                                       field.count);
      }
    }
  }
