...
hdf5_to_msg::hdf5_to_msg() {
  ...
  srv_get_geometry_vertex_normals_ = node_handle.advertiseService(
      "get_geometry_vertexnormals",
      &hdf5_to_msg::service_getGeometryVertexNormals, this);
  ...
  srv_get_vertex_cost_layers_ = node_handle.advertiseService(
      "get_vertex_cost_layers", &hdf5_to_msg::service_getVertexCostLayers,
      this);

  pub_geometry_ = node_handle.advertise<mesh_msgs::MeshGeometryStamped>(
      "mesh/geometry", 1, true);
  pub_vertex_colors_ =
      node_handle.advertise<mesh_msgs::MeshVertexColorsStamped>(
          "mesh/vertex_colors", 1, true);
  pub_vertex_costs_ = node_handle.advertise<mesh_msgs::MeshVertexCostsStamped>(
      "mesh/vertex_costs", 1);

  srv_get_labeled_clusters_ = node_handle.advertiseService(
      "get_labeled_clusters", &hdf5_to_msg::service_getLabeledClusters, this);

  sub_cluster_label_ = node_handle.subscribe(
      "cluster_label", 10, &hdf5_to_msg::callback_clusterLabel, this);
...
bool hdf5_to_msg::getVertices(std::vector<float> &vertices,
                              mesh_msgs::MeshGeometryStamped &geometryMsg) {
...
bool hdf5_to_msg::getFaces(std::vector<uint32_t> &faceIds,
                           mesh_msgs::MeshGeometryStamped &geometryMsg) {
...
bool hdf5_to_msg::getVertexColors(
  ...
  for (unsigned int i = 0; i < nVertices; i++) {
    vertexColorsMsg.mesh_vertex_colors.vertex_colors[i].r =
        vertexColors[i * 3] / 255.0f;
    vertexColorsMsg.mesh_vertex_colors.vertex_colors[i].g =
        vertexColors[i * 3 + 1] / 255.0f;
    vertexColorsMsg.mesh_vertex_colors.vertex_colors[i].b =
        vertexColors[i * 3 + 2] / 255.0f;
    vertexColorsMsg.mesh_vertex_colors.vertex_colors[i].a = 1;
  }
...
bool hdf5_to_msg::getVertexCosts(
    std::vector<float> &costs, std::string layer,
    mesh_msgs::MeshVertexCostsStamped &vertexCostsMsg) {
...
bool hdf5_to_msg::service_getUUIDs(mesh_msgs::GetUUIDs::Request &req,
                                   mesh_msgs::GetUUIDs::Response &res) {
  res.uuids.push_back(mesh_uuid);
  return true;
}

bool hdf5_to_msg::service_getGeometry(mesh_msgs::GetGeometry::Request &req,
...
bool hdf5_to_msg::service_getMaterials(mesh_msgs::GetMaterials::Request &req,
                                       mesh_msgs::GetMaterials::Response &res) {
  hdf5_map_io::HDF5MapIO io(inputFile);

  // Materials
  auto materials = io.getMaterials();
  auto materialFaceIndices =
      io.getMaterialFaceIndices(); // for each face: material index
  unsigned int nMaterials = materials.size();
  unsigned int nFaces = materialFaceIndices.size();
  ROS_INFO_STREAM("Found " << nMaterials << " materials and " << nFaces
                           << " faces");
  ...
  for (uint32_t i = 0; i < nMaterials; i++) {
    ...
    // has texture
    res.mesh_materials_stamped.mesh_materials.materials[i].has_texture =
        texture_index >= 0;

    // texture index
    res.mesh_materials_stamped.mesh_materials.materials[i].texture_index =
        static_cast<uint32_t>(texture_index);
    // color
    res.mesh_materials_stamped.mesh_materials.materials[i].color.r =
        materials[i].r / 255.0f;
    res.mesh_materials_stamped.mesh_materials.materials[i].color.g =
        materials[i].g / 255.0f;
    res.mesh_materials_stamped.mesh_materials.materials[i].color.b =
        materials[i].b / 255.0f;
    res.mesh_materials_stamped.mesh_materials.materials[i].color.a = 1;
  }
  ...
  for (uint32_t i = 0; i < nFaces; i++) {
    ...
    if (materialToFaces.count(materialIndex) == 0) {
      materialToFaces.insert(
          std::make_pair(materialIndex, std::vector<uint32_t>()));
    }
  ...
  for (uint32_t i = 0; i < nMaterials; i++) {
    for (uint32_t j = 0; j < materialToFaces[i].size(); j++) {
      res.mesh_materials_stamped.mesh_materials.clusters[i]
          .face_indices.push_back(materialToFaces[i][j]);
    }
  }
  res.mesh_materials_stamped.mesh_materials.cluster_materials.resize(
      nMaterials);
  for (uint32_t i = 0; i < nMaterials; i++) {
  ...
  for (uint32_t i = 0; i < nVertices; i++) {
    // coords: u/v/w
    // w is always 0
    res.mesh_materials_stamped.mesh_materials.vertex_tex_coords[i].u =
        vertexTexCoords[3 * i];
    res.mesh_materials_stamped.mesh_materials.vertex_tex_coords[i].v =
        vertexTexCoords[3 * i + 1];
  }
...
bool hdf5_to_msg::service_getTexture(mesh_msgs::GetTexture::Request &req,
  ...
  for (auto texture : io.getTextures()) {
    if (std::stoi(texture.name) == req.texture_index) {
      ...
      sensor_msgs::fillImage( // TODO: only RGB, breaks when using other color
                              // channels
          image, "rgb8", texture.height, texture.width,
...
void hdf5_to_msg::callback_clusterLabel(
    const mesh_msgs::MeshFaceClusterStamped::ConstPtr &msg) {
  ...
  boost::split(split_results, msg->cluster.label,
               [](char c) { return c == '_'; });
