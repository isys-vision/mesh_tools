...
LabelManager::LabelManager(ros::NodeHandle &nodeHandle) : nh(nodeHandle) {
  ...
  clusterLabelSub = nh.subscribe("cluster_label", 10,
                                 &LabelManager::clusterLabelCallback, this);
  newClusterLabelPub =
      nh.advertise<mesh_msgs::MeshFaceCluster>("new_cluster_label", 1);
  srv_get_labeled_clusters = nh.advertiseService(
      "get_labeled_clusters", &LabelManager::service_getLabeledClusters, this);
  srv_get_label_groups = nh.advertiseService(
      "get_label_groups", &LabelManager::service_getLabelGroups, this);
  srv_get_labeled_cluster_group =
      nh.advertiseService("get_labeled_cluster_group",
                          &LabelManager::service_getLabeledClusterGroup, this);
  srv_delete_label = nh.advertiseService(
      "delete_label", &LabelManager::service_deleteLabel, this);
...
void LabelManager::clusterLabelCallback(
    const mesh_msgs::MeshFaceClusterStamped::ConstPtr &msg) {
  ROS_INFO_STREAM("Got msg for mesh: " << msg->uuid << " with label: "
                                       << msg->cluster.label);
  ...
  if (!msg->override) {
    ...
    if (readIndices.empty()) {
      ...
      for (size_t i = 0; i < msg->cluster.face_indices.size(); i++) {
        ...
        if (std::find(readIndices.begin(), readIndices.end(), idx) ==
            readIndices.end()) {
...
bool LabelManager::service_getLabelGroups(
  ...
  for (directory_iterator itr(p); itr != end_itr; ++itr) {
    ...
    if (is_regular_file(itr->path())) {
      ...
      if (std::find(res.labels.begin(), res.labels.end(), label) ==
          res.labels.end()) {
...
bool LabelManager::service_deleteLabel(
  ...
  if (!is_regular_file(p) || !exists(p)) {
    ROS_WARN_STREAM("Could not delete label '" << req.label << "' of mesh '"
                                               << req.uuid << "'.");
...
bool LabelManager::service_getLabeledClusterGroup(
  ...
  for (directory_iterator itr(p); itr != end_itr; ++itr) {
    // if file is no dir
    if (is_regular_file(itr->path()) &&
        itr->path().filename().string().find(req.labelGroup) == 0) {
      std::string label = itr->path().filename().string();
      // remove extension from label
      boost::replace_all(label, itr->path().filename().extension().string(),
                         "");
...
bool LabelManager::writeIndicesToFile(const std::string &fileName,
...
std::vector<uint>
LabelManager::readIndicesFromFile(const std::string &fileName) {
  std::ifstream ifs(fileName.c_str(), std::ios::in);
  std::vector<uint> faceIndices;

  // if file dos not exists, return empty vector
  if (!ifs.good()) {
    ROS_DEBUG_STREAM("File " << fileName
...
std::string LabelManager::getFileName(const std::string &uuid,
