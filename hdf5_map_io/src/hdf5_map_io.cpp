...
HDF5MapIO::HDF5MapIO(std::string filename, const std::vector<float> &vertices,
                     const std::vector<uint32_t> &face_ids)
    : m_file(filename,
             hf::File::ReadWrite | hf::File::Create | hf::File::Truncate) {
...
std::vector<MapMaterial> HDF5MapIO::getMaterials() {
  ...
  m_texturesGroup.getDataSet("materials").read(materials);
...
std::vector<uint32_t> HDF5MapIO::getMaterialFaceIndices() {
  ...
  m_texturesGroup.getDataSet("mat_face_indices").read(matFaceIndices);
...
std::vector<float> HDF5MapIO::getVertexTextureCoords() {
  ...
  m_texturesGroup.getDataSet("coords").read(coords);
...
std::vector<uint32_t> HDF5MapIO::getFaceIdsOfLabel(std::string groupName,
...
std::vector<float> HDF5MapIO::getVertexCosts(std::string costlayer) {
  ...
  m_channelsGroup.getDataSet(costlayer).read(costs);
...
MapImage HDF5MapIO::getImage(hf::Group group, std::string name) {
  ...
  H5IMget_image_info(group.getId(), name.c_str(), &width, &height, &pixel_size,
                     interlace, &npals);
...
hf::DataSet HDF5MapIO::addVertexNormals(std::vector<float> &normals) {
  ...
  auto dataSet = m_channelsGroup.createDataSet<float>(
      "vertex_normals", hf::DataSpace::From(normals));
...
hf::DataSet HDF5MapIO::addVertexColors(std::vector<uint8_t> &colors) {
  auto dataSet = m_channelsGroup.createDataSet<uint8_t>(
      "vertex_colors", hf::DataSpace::From(colors));
  dataSet.write(colors);

  return dataSet;
}

void HDF5MapIO::addTexture(int index, uint32_t width, uint32_t height,
                           uint8_t *data) {
  if (!m_texturesGroup.exist("images")) {
...
void HDF5MapIO::addMaterials(std::vector<MapMaterial> &materials,
                             std::vector<uint32_t> &matFaceIndices) {
  m_texturesGroup
  ...
  m_texturesGroup
      .createDataSet<uint32_t>("mat_face_indices",
                               hf::DataSpace::From(matFaceIndices))
      .write(matFaceIndices);
}

void HDF5MapIO::addVertexTextureCoords(std::vector<float> &coords) {
  m_texturesGroup.createDataSet<float>("coords", hf::DataSpace::From(coords))
      .write(coords);
}

void HDF5MapIO::addOrUpdateLabel(std::string groupName, std::string labelName,
                                 std::vector<uint32_t> &faceIds) {
  ...
  if (group.exist(labelName)) {
    ...
    auto dataset =
        group.createDataSet<uint32_t>(labelName, hf::DataSpace::From(faceIds));
    dataset.write(faceIds);
  }
}

void HDF5MapIO::addLabel(std::string groupName, std::string labelName,
                         std::vector<uint32_t> &faceIds) {
  if (!m_labelsGroup.exist(groupName)) {
    m_labelsGroup.createGroup(groupName);
  }
...
void HDF5MapIO::addTextureKeypointsMap(
  ...
  for (const auto &keypoint_features : keypoints_map) {
    auto dataset = tf.createDataSet<float>(
        std::to_string(i), hf::DataSpace::From(keypoint_features.second));
    dataset.write(keypoint_features.second);

    std::vector<float> v = {keypoint_features.first.x,
                            keypoint_features.first.y,
                            keypoint_features.first.z};
...
void HDF5MapIO::addRoughness(std::vector<float> &roughness) {
  m_channelsGroup
      .createDataSet<float>("roughness", hf::DataSpace::From(roughness))
      .write(roughness);
}
...
void HDF5MapIO::addImage(hf::Group group, std::string name,
                         const uint32_t width, const uint32_t height,
                         const uint8_t *pixelBuffer) {
  H5IMmake_image_24bit(group.getId(), name.c_str(), width, height,
...
void HDF5MapIO::flush() { m_file.flush(); }
